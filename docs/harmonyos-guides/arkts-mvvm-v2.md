...
指南应用框架[ArkUI（方舟UI框架）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkui)[UI开发 (ArkTS声明式开发范式)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-development)[学习UI范式状态管理](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management)MVVM模式（V2）
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6f/v3/kgcdnYeDSjmbIgpCAQeX_g/zh-cn_image_0000002526144182.gif?HW-CC-KV=V1&HW-CC-Date=20260311T194404Z&HW-CC-Expire=86400&HW-CC-Sign=6817EEBA79D007C1A1503E5B0E56B6F0D8FD29D41E57D38EE2179202F967E104)
...
-   ListView：负责展示任务列表，并根据Setting中的设置筛选是否显示已完成的任务。它依赖于TaskListViewModel来获取任务数据，并通过TaskItem组件进行渲染，包括任务的名称、完成状态以及删除按钮。通过TaskViewModel和TaskListViewModel实现用户的交互，如切换任务完成状态和删除任务。
    ...
    ```TypeScript
      ...
      12.    @Monitor('task.isFinish')
      13.    onTaskFinished(mon: IMonitor) {
      14.      hilog.info(0x0000, 'testTag', '%{public}s', 'task' + this.task.taskName + 'The completion status of the' + mon.value()?.before + 'has become' + mon.value()?.now);
      15.    }
      
      17.    build() {
      18.      Row() {
      19.        // 请开发者自行在src/main/resources/base/media路径下添加finished.png和unfinished.png两张图片，否则运行时会因资源缺失而报错。
      20.        Image(this.task.isFinish ? $r('app.media.finished') : $r('app.media.unfinished'))
      21.          .width(28)
      22.          .height(28)
      23.          .margin({ left: 15, right: 10 })
      24.        Text(this.task.taskName)
      25.          .decoration({ type: this.task.isFinish ? TextDecorationType.LineThrough : TextDecorationType.None })
      26.          .fontSize(18)
      27.        ActionButton('Delete', () => this.deleteTask());
      28.      }
      29.      .height('7%')
      30.      .width('90%')
      31.      .backgroundColor('#90f1f3f5')
      32.      .borderRadius(25)
      33.      .onClick(() => this.task.updateIsFinish())
      34.    }
      35.  }
      
      37.  @ComponentV2
      38.  export default struct ListView {
      39.    @Param taskList: TaskListViewModel = new TaskListViewModel();
      40.    @Param setting: Setting = new Setting();
      
      42.    build() {
      43.      Repeat<TaskViewModel\>(this.taskList.tasks.filter(task => this.setting.showCompletedTask || !task.isFinish))
      44.        .each((obj: RepeatItem<TaskViewModel>) => {
      45.          TaskItem({
      46.            task: obj.item,
      47.            deleteTask: () => this.taskList.removeTask(obj.item)
      48.          }).margin(5)
      49.        })
      50.    }
      51.  }
...
-   BottomView：负责提供与任务操作相关的按钮和输入框，如"全部完成"、"全部未完成"，"设置"三个按钮，以及添加新任务的输入框。点击"全部完成"和"全部未完成"时，通过TaskListViewModel更改所有任务的状态。点击"设置"按钮时，会导航到SettingAbility的设置页面。添加新任务时，通过TaskListViewModel新增任务到任务列表中。
    ...
    ```TypeScript
      ...
      6.  @Builder export function ActionButton(text: string|Resource, onClick:() => void) {
      7.    Button(text, { buttonStyle: ButtonStyleMode.NORMAL })
      8.      .onClick(onClick)
      9.      .margin({ left: 10, right: 10, top: 5, bottom: 5 })
      10.  }
      
      12.  @ComponentV2
      13.  export default struct BottomView {
      14.    @Param taskList: TaskListViewModel = new TaskListViewModel();
      15.    @Local newTaskName: string = '';
      16.    private context = this.getUIContext().getHostContext() as common.UIAbilityContext;
      
      18.    build() {
      19.      Column() {
      20.        Row() {
      21.          ActionButton('All Completed', (): void => this.taskList.finishAll(true))
      22.          ActionButton('All Not Completed', (): void => this.taskList.finishAll(false))
      23.        }
      24.        .margin({ top: 10 })
      25.        Row(){
      26.          ActionButton('Setting', (): void => {
      27.            let wantInfo: Want = {
      28.              deviceId: '', // deviceId为空表示本设备。
      29.              bundleName: 'com.samples.statemgmtv2mvvm', // 替换成AppScope/app.json5里的bundleName。
      30.              abilityName: 'SettingAbility',
      31.            };
      ...
      35.        .margin({ bottom: 5 })
      36.        Row() {
      37.          TextInput({ placeholder: 'Add new tasks', text: this.newTaskName })
      38.            .onChange((value) => this.newTaskName = value)
      39.            .width('70%')
      40.          ActionButton('+', (): void => {
      41.            let newTask = new TaskViewModel();
      42.            newTask.taskName = this.newTaskName;
      43.            this.taskList.addTask(newTask);
      44.            this.newTaskName = '';
      45.          })
      46.        }
      47.      }
...
[MVVM模式（V1）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm "MVVM模式（V1）")

[状态管理（V1）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management-v1 "状态管理（V1）")
...
_文档_[知识问答](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-code-generation "知识问答")
...
_文档_[DialogV2](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ohos-arkui-advanced-dialogv2 "DialogV2")

_文档_[MVVM模式（V1）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm "MVVM模式（V1）")

_文档_[状态管理最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-status-management "状态管理最佳实践")

_文档_[Worker和TaskPool的线程数量是否有限制](https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs/faqs-arkts-28 "Worker和TaskPool的线程数量是否有限制")

_文档_[AppStorageV2: 应用全局UI状态存储](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-appstoragev2 "AppStorageV2: 应用全局UI状态存储")

_视频课程_[54- V2类属性变化检测与跨组件层级双向同步](https://developer.huawei.com/consumer/cn/training/course/introduction/C401770192543265627 "54- V2类属性变化检测与跨组件层级双向同步")

_视频课程_[ArkTS语法介绍](https://developer.huawei.com/consumer/cn/training/course/introduction/C101717496870909384 "ArkTS语法介绍")
...
[概述](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#概述)

[通过状态管理V2版本实现ViewModel](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#通过状态管理v2版本实现viewmodel)

[基础示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#基础示例)

[添加@Local，实现对组件内部状态观测](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加local实现对组件内部状态观测)

[添加@Param，实现组件接收外部输入](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加param实现组件接收外部输入)

[添加@Event，实现组件对外输出](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加event实现组件对外输出)

[添加Repeat，实现子组件复用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加repeat实现子组件复用)

[添加@ObservedV2，@Trace，实现类属性观测变化](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加observedv2trace实现类属性观测变化)

[添加@Monitor，@Computed，实现监听状态变量和计算属性](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加monitorcomputed实现监听状态变量和计算属性)

[添加AppStorageV2，实现应用全局UI状态存储](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加appstoragev2实现应用全局ui状态存储)

[添加PersistenceV2，实现持久化UI状态存储](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加persistencev2实现持久化ui状态存储)

[添加@Builder，实现自定义构建函数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#添加builder实现自定义构建函数)

[效果图展示](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#效果图展示)

[重构代码以符合MVVM架构](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#重构代码以符合mvvm架构)

[重构后的代码结构](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#重构后的代码结构)

[Model层](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#model层)

[ViewModel层](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#viewmodel层)

[View层](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#view层)

[总结](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#总结)

[代码示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-mvvm-v2#代码示例)
